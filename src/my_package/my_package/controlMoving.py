...
class XgoGait(rclpy.node.Node):
    def __init__(self):
        ...
        self.gait_type = self.get_parameter(
            'gait').get_parameter_value().string_value
        self.get_logger().info('gait_type: %s!' % self.gait_type)
        self.mark = 0
        self.declare_parameter('marking', self.mark)
        self.mark_type = self.get_parameter(
            'marking').get_parameter_value().integer_value
    ...
    def cmdvel_callback(self, msg):
        ...
        # 限幅
        if self.vel_x_ > self.MAXVALUE:
            self.vel_x_ = self.MAXVALUE
        if self.vel_y_ > self.MAXVALUE:
            self.vel_y_ = self.MAXVALUE
        if self.vel_x_ < -self.MAXVALUE:
            self.vel_x_ = -self.MAXVALUE
        if self.vel_y_ < -self.MAXVALUE:
            self.vel_y_ = -self.MAXVALUE
        if self.angular_z_ > self.MAXVALUE:
            self.angular_z_ = self.MAXVALUE
        if self.angular_z_ < -self.MAXVALUE:
            self.angular_z_ = -self.MAXVALUE
        # end 限幅
        # set dog motion
